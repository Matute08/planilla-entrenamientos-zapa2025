...
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbygckn2NLCu0z7XvSbi1HtqOWdfdxX7QzhkwypKc20FEP4rr4Tcj9rvugrZHQTBpfQ2/exec'; // Pega tu URL /exec aquí
...
function Planilla() {
  ...
  const [selectedMonthIndex, setSelectedMonthIndex] = useState(new Date().getMonth()); // Inicia con el mes actual
  ...
  const fetchData = useCallback(async (monthIndex) => {
    if (!SCRIPT_URL || SCRIPT_URL === 'URL_DE_TU_APPS_SCRIPT_AQUI') {
        setError("Error: La URL de Google Apps Script no está configurada en el código React.");
        setLoading(false);
        setPlayers([]);
        setTrainingDates([]);
        return;
    ...
    try {
      ...
      if (data.error) {
          throw new Error(data.message || 'Error desconocido desde Apps Script.');
      }

      // Verifica que los datos recibidos tengan la estructura esperada
      if (data && Array.isArray(data.players) && Array.isArray(data.trainingDates)) {
  ...
  const updateData = async (payload) => {
     if (!SCRIPT_URL || SCRIPT_URL === 'URL_DE_TU_APPS_SCRIPT_AQUI') {
        setError("Error: La URL de Google Apps Script no está configurada.");
...
return (
    <div className="flex flex-col h-screen bg-blue-400 font-sans">
        <header className="text-black text-center py-4">
            <h1 className="text-4xl font-bold ">Planilla de Asistencias</h1>
        </header>

        <main className="flex-grow overflow-auto p-4">
            {/* Selector de Mes */}
            <div className="mb-6 flex justify-center items-center space-x-2">
                <label htmlFor="month-select" className="text-xl font-medium text-black">Mes:</label>
                <select
                    id="month-select"
                    value={selectedMonthIndex}
                    onChange={handleMonthChange}
                    disabled={loading}
                    className="text-xl mt-1 block w-auto pl-3 pr-10 py-2 border-gray-600 focus:outline-none focus:border-blue-500 sm:text-sm rounded-md shadow-sm disabled:bg-gray-200 "
                >
                ...
                <span className="text-xl font-semibold text-black">{new Date().getFullYear()}</span>
            </div>

            {/* Indicador de Carga */}
            {loading && (
                <div className="text-center p-4 text-xl text-black">
                    <svg className="animate-spin h-6 w-6 inline-block mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            {/* Mensaje de Error */}
            {error && (
                <div className="mb-4 p-3 rounded-md bg-red-100 text-black text-center shadow">
                    <strong>{error}</strong> 
                </div>
            )}
                ...
                <div className="overflow-x-auto bg-white rounded-lg shadow-md">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider sticky left-0 bg-gray-50 z-10">
                                    Nombre Jugador
                                </th>
                                {trainingDates.map((date, index) => (
                                    <th key={index} scope="col" className="px-6 py-3 text-center text-xs font-medium text-black uppercase tracking-wider">
                                        {(() => {
                                            try {
                                                return new Date(date + 'T00:00:00').toLocaleDateString('es-AR', { month: 'short', day: 'numeric' });
                                            } catch (e) {
                                                return date;
                                            }
                                        })()}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {players.map((player) => (
                                <tr key={player.id} className="hover:bg-gray-50">
                                    ...
                                    {trainingDates.map((_, dateIndex) => (
                                        <td key={dateIndex} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                            <input
                                                type="checkbox"
                                                checked={player.attendance[dateIndex] || false}
                                                onChange={() => handleAttendanceChange(player.id, dateIndex)}
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                        ...
                                        />
                                        <span className={`ml-2 text-xs font-semibold ${player.paid ? 'text-green-700' : 'text-red-700'}`}>
                            ...
                            {players.length === 0 && (
                                <tr>
                                    <td colSpan={trainingDates.length + 2} className="px-6 py-4 text-center text-black">
                                        No hay jugadores para mostrar en {months[selectedMonthIndex]}. Verifica la hoja de cálculo o selecciona otro mes.
                                    </td>
                                </tr>
                            )}
                        </tbody>
